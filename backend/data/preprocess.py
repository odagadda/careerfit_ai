...
# 현재 파일(preprocess.py)이 있는 폴더 경로
# 예: C:\오다빈\Git\careerfit_ai\backend\data
...
def load_data(filepath: str) -> pd.DataFrame:
    ...
    요리 비유: 재료 목록표(jobs.csv)를 주방으로 가져오는 단계입니다.
    """
...
def handle_missing(df: pd.DataFrame) -> pd.DataFrame:
    ...
    - 핵심 컬럼(title, required_skills)이 비어있는 행은 제거합니다.
    - 나머지 텍스트 컬럼은 빈 문자열로 채웁니다.

    요리 비유: 꼭 필요한 재료가 빠진 요리는 제외하고,
    덜 중요한 빈칸은 빈 접시로 정리하는 단계입니다.
    """
    ...
    # 핵심 컬럼이 비어있는 행 제거
    # title: 어떤 직무인지
    # required_skills: 어떤 필수 역량이 필요한지
    # 이 둘이 없으면 RAG 검색에 의미가 약함
    df = df.dropna(subset=["title", "required_skills"])

    # 나머지 텍스트 컬럼은 빈 문자열로 채우기
...
def remove_duplicates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    요리 비유: 같은 재료가 장바구니에 두 번 들어갔는지 확인하고,
    하나만 남기는 단계입니다.
    """
...
def normalize_skills(skills_str: str) -> str:
    ...
    요리 비유: 같은 재료인데 이름표가 다르게 붙은 것을
    하나의 표준 이름으로 정리하는 단계입니다.
    """
    ...
    for skill in skills:
        ...
        # 사전에 있으면 표준 이름으로 변환하고, 없으면 원래 값 유지
...
def standardize_skills(df: pd.DataFrame) -> pd.DataFrame:
    """
    required_skills, preferred_skills 컬럼 전체에 스킬 표준화를 적용합니다.

    요리 비유: 재료 이름을 제각각 적어둔 장바구니를
    보기 좋게 라벨링하는 단계입니다.
    """
    ...
    # 표준화를 적용할 컬럼 목록
    skill_cols = ["required_skills", "preferred_skills"]
    ...
    for col in ["title", "required_skills", "preferred_skills"]:
        if col in df.columns:
